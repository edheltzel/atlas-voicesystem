 ...
 * the voice server for spoken playback.
 *
 * TRIGGER: Stop
 *
 ...
 *
 * Repo-owned (echo) so the Claude Code adapter owns the per-turn voice
...
import { extractVoiceCompletion } from './lib/TranscriptParser';
...
async function main() {
  ...
  // Persona detection reads the response text (🗣️ <Name>: tag). If the transcript
  // parse lagged and produced no current-turn text, fall back to the hook input's
  // last_assistant_message so a persona is still recognized.
  if (!parsed.currentResponseText && input.last_assistant_message) {
    parsed.currentResponseText = input.last_assistant_message;
  }
